...
MeteoNode is a small solar-powered ESP32 weather node. The basic idea is to pull regional forecast data from Open-Meteo and then adjust it using pressure, humidity, and temperature trends measured locally by the node itself. Instead of sending the data somewhere else to process, the ESP32 handles everything on-device using a lightweight Bayesian-style system.
