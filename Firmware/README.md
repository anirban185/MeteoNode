...
The firmware uses `client.setInsecure()` to connect to Open-Meteo over HTTPS. The connection is encrypted but the server identity is not verified because pinning a certificate on an ESP32 is more complexity than this project needs. Fine for a local weather node on a home network, but worth knowing if you're deploying it somewhere else.
